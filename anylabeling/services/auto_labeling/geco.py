...
class GeCoRunAllImagesONNX(GeCoONNX):
    """zero shot count model using GeCo"""

    def __init__(
        self, encoder_model_path, protos_model_path, decoder_model_path, input_size, box_threshold
    ) -> None:
        self.input_size = input_size
        self.box_threshold = box_threshold
        ...
        self.protos_session = ort.InferenceSession(
            protos_model_path,
            providers=providers,
            sess_options=sess_options,
        )
    ...
    def run_protos(
        ...
        outputs = self.protos_session.run(None, protos_inputs)

        prototype_embeddings = outputs[0]
    ...
    def get_protos(self, embedding, prompt):
        return self.run_protos(
            embedding["image_embeddings"],
            embedding["scale_factor"],
            prompt
        )

    def predict_bboxes(self, embedding, prototype_embeddings):
        """
        Predict masks for a single image.
        """
        bboxes = self.run_decoder(
...
class GeCoRunAllImages(GeCo):
    ...
    def __init__(self, config_path, on_message) -> None:
        ...
        protos_model_abs_path = self.get_model_abs_path(
            self.config, "protos_model_path"
        )
        if not protos_model_abs_path or not os.path.isfile(
            protos_model_abs_path
        ):
            raise FileNotFoundError(
                QCoreApplication.translate(
                    "Model",
                    "Could not download or initialize protos of GeCo.",
                )
            )
        ...
        # Load models
        self.model = GeCoRunAllImagesONNX(
            encoder_model_abs_path,
            protos_model_abs_path,
            decoder_model_abs_path,
            self.input_size,
    ...
    def set_auto_labeling_reset_tracker(self):
        ...
        if self.prototype_embeddings:
            try:
                self.prototype_embeddings = None
                logger.info(
                    "Successful: The protos has been reset to its initial state."
                )
            except Exception as e:  # noqa
                pass
