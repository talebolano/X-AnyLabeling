...
class GeCo:
    ...
    def __init__(self, model_config=None) -> None:
        ...
        protos_path = self.config["protos_path"]
        ...
        if not protos_path or not os.path.isfile(protos_path):
            raise FileNotFoundError(print("file not found: ", protos_path))
        ...
        self.protos = OnnxBaseModel(
            protos_path, device_type=self.config["device"]
        )
    ...
    def predict_shapes(self, image, box_prompt, image_path=None):
        ...
        prototype_embeddings = self.protos.get_ort_inference(
            None, inputs=protos_inputs,extract=False
        )[0]
...
def export_onnx(model, output_encoder_file, output_protos_file, output_decoder_file, is_quantize):
    # export encoder
    tmp_dir = mkdtemp()
    ...
    # export protos
    def protos_forward(self, src, bboxes):
    ...
    model.forward = MethodType(protos_forward, model)
    ...
    with torch.no_grad():
        torch.onnx.export(
            ...
            output_names=["prototype_embeddings"],
    ...
    # export decdoer
    def decoder_forward(self, src, src_hq, prototype_embeddings):
    ...
    dummy_inputs = {
        ...
        "prototype_embeddings": torch.randn(size=(1, 2, 256), dtype=torch.float),
    }

    dynamic_axes = {
        "prototype_embeddings": {1: "num_bboxes"},
    }
...
if __name__ == "__main__":
    ...
    onnx_protos_file = (
        osp.splitext(osp.basename(ckpt_file))[0] + "_protos.onnx"
    )
    ...
    if not (osp.exists(onnx_encoder_file) and osp.exists(onnx_decoder_file)):
        ...
        # export model
        export_onnx(model, onnx_encoder_file, onnx_protos_file, onnx_decoder_file, is_quantize)

    # inference on a image and test the speed
    configs = {
        "encoder_path": onnx_encoder_file,
        "protos_path": onnx_protos_file,
